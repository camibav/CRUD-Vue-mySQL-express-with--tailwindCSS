...
const cambioDePagina = () => {
  paginaActual.value = parseInt(event.target.textContent)//parseInt convierte el string en un número entero
}
</script>
<template>
  <div class="flex justify-center min-h-screen bg-black">
    <div class="w-full max-w-5xl px-4 py-8">
      ...
      <input v-model="terminoBúsqueda" type="text"
      ...
      <table class="table text-yellow-200 w-full bg-gray-900  rounded-xl ">
        ...
        <tbody>
          <tr v-for="usuario in usuariosPaginados" :key="usuario.id"
            class="bg-gray-800 hover:bg-gray-700 transition-colors">
            <td class="px-6 py-4 ">{{ usuario.id }}</td>
            <td class="px-6 py-4 ">{{ usuario.nombre }}</td>
            <td class="px-6 py-4 w-1/2">{{ usuario.email }}</td>
            <td class="px-6 py-4 ">
              <button class="bg-yellow-500 text-black p-2 rounded-md mx-2 hover:bg-blue-400 "
                @click="editarUsuario(usuario.id)">Editar</button>
              <button class="bg-red-400 text-black p-2 rounded-md mx-2 hover:bg-cyan-400  "
                @click="borrarUsuario(usuario.id)">Eliminar</button>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="flex justify-center mt-4">
        <span v-for="(item, index) in totalPaginas" :key="index"
          class="flex items-center justify-center transform transition-all duration-500 ease-in-out hover:scale-110">
          <button @click="cambioDePagina"
            class="border-[1px] text-yellow-200 border-gray-500 py-1 px-2 mx-[1px] text-[1rem] hover:bg-yellow-200 rounded-md hover:text-black shadow-md hover:shadow-lg active:scale-90 transition duration-150 ease-in-out">{{
              index + 1 }} </button>
        </span>
      </div>
    </div>
